...
inline uint32_t hash_fnv1a(const string& str) {
...
inline Type property_type_from_string(const string& t) {
  if (t == "int8" || t == "char") {
    return Type::INT8;
...
struct PlyFile::PlyFileImpl {
  ...
  struct ParsingHelper {
    shared_ptr<PlyData> data;
    shared_ptr<PlyCursor> cursor;
  };

  unordered_map<uint32_t, ParsingHelper> userData;
  ...
  void read(istream& is, uint32_t fixed_list_size);
  void write(ostream& os, bool isBinary);

  shared_ptr<PlyData> request_properties_from_element(
      const string& elementKey,
      const initializer_list<string>& propertyKeys);
  ...
  size_t read_property_ascii(
      ...
      size_t& destOffset,
      istream& is);
  size_t skip_property_binary(const PlyProperty& property, istream& is);
  size_t skip_property_ascii(const PlyProperty& property, istream& is);

  bool parse_header(istream& is);
  void parse_data(istream& is, bool firstPass);
  void read_header_format(istream& is);
  void read_header_element(istream& is);
  void read_header_property(istream& is);
  ...
  void write_ascii_internal(ostream& os);
  void write_binary_internal(ostream& os);
  void
  write_property_ascii(Type t, ostream& os, uint8_t* src, size_t& srcOffset);
  void
  write_property_binary(Type t, ostream& os, uint8_t* src, size_t& srcOffset);
};
...
PlyProperty::PlyProperty(istream& is) : isList(false) {
  string type;
  is >> type;
  if (type == "list") {
    string countType;
...
void ply_cast(void* dest, const char* src, bool be) {
  *(static_cast<T*>(dest)) = (be)
      ? endian_swap(*(reinterpret_cast<const T*>(src)))
...
void ply_cast_float(void* dest, const char* src, bool be) {
  *(static_cast<T*>(dest)) = (be)
      ? endian_swap_float(*(reinterpret_cast<const uint32_t*>(src)))
      : *(reinterpret_cast<const T*>(src));
}
...
void ply_cast_double(void* dest, const char* src, bool be) {
  *(static_cast<T*>(dest)) = (be)
      ? endian_swap_double(*(reinterpret_cast<const uint64_t*>(src)))
      : *(reinterpret_cast<const T*>(src));
}

template <typename T>
T ply_read_ascii(istream& is) {
  T data;
  is >> data;
  return data;
}
...
template <typename T>
void ply_cast_ascii(void* dest, istream& is) {
  *(static_cast<T*>(dest)) = ply_read_ascii<T>(is);
}

int64_t find_element(const string& key, const vector<PlyElement>& list) {
...
int64_t find_property(const string& key, const vector<PlyProperty>& list) {
  for (size_t i = 0; i < list.size(); ++i) {
    if (list[i].name == key) {
      return i;
    }
...
static map<Type, PropertyInfo> PropertyTable{{Type::INT8, {1, "char"}},
                                             {Type::UINT8, {1, "uchar"}},
                                             {Type::INT16, {2, "short"}},
...
bool PlyFile::PlyFileImpl::parse_header(istream& is) {
  string line;
  while (getline(is, line)) {
    ...
    if (token == "ply" || token == "PLY" || token == "") {
      ...
      return false; // unexpected header field
    }
  }
  return true;
}

void PlyFile::PlyFileImpl::read_header_text(
    string line,
    istream& /* is */,
    vector<string>& place,
    int erase) {
...
void PlyFile::PlyFileImpl::read_header_format(istream& is) {
  string s;
  (is >> s);
...
void PlyFile::PlyFileImpl::read_header_element(istream& is) {
  elements.emplace_back(is);
}

void PlyFile::PlyFileImpl::read_header_property(istream& is) {
  if (!elements.size()) {
    throw runtime_error("no elements defined; file is malformed");
  }
  elements.back().properties.emplace_back(is);
}

size_t PlyFile::PlyFileImpl::skip_property_binary(
    const PlyProperty& p,
    istream& is) {
...
size_t PlyFile::PlyFileImpl::skip_property_ascii(
    const PlyProperty& p,
    istream& is) {
  const int32_t stride = PropertyTable[p.propertyType].stride;
...
size_t PlyFile::PlyFileImpl::read_property_binary(
    const Type t,
    void* dest,
    size_t& destOffset,
    istream& is) {
  ...
  switch (t) {
    case Type::INT8:
      ply_cast<int8_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::UINT8:
      ply_cast<uint8_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::INT16:
      ply_cast<int16_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::UINT16:
      ply_cast<uint16_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::INT32:
      ply_cast<int32_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::UINT32:
      ply_cast<uint32_t>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::FLOAT32:
      ply_cast_float<float>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::FLOAT64:
      ply_cast_double<double>(dest, (char*)scratch, isBigEndian);
      break;
    case Type::INVALID:
      throw invalid_argument("invalid ply property");
  }

  return stride;
}

size_t PlyFile::PlyFileImpl::read_property_ascii(
    const Type t,
    void* dest,
    size_t& destOffset,
    istream& is) {
...
void PlyFile::PlyFileImpl::write_property_ascii(
    Type t,
    ostream& os,
    uint8_t* src,
    size_t& srcOffset) {
...
void PlyFile::PlyFileImpl::read(istream& is, uint32_t fixed_list_size) {
  // Parse but only get the data size
  if (fixed_list_size == 0) {
    parse_data(is, true);
  }

  vector<shared_ptr<PlyData>> buffers;
  for (auto& entry : userData) {
    buffers.push_back(entry.second.data);
  }

  // Count the number of properties (required for allocation)
  unordered_map<PlyData*, int32_t> uniqueCount;
  for (auto& ptr : buffers) {
    uniqueCount[ptr.get()] += 1;
  }

  // Since group-requested properties share the same cursor, we need to find
  // unique cursors so we only allocate once
  sort(buffers.begin(), buffers.end());
  buffers.erase(unique(buffers.begin(), buffers.end()), buffers.end());

  // Not great, but since we sorted by ptrs on PlyData, need to remap back
  // onto its cursor in the userData table
  for (auto& b : buffers) {
    for (auto& entry : userData) {
      if (entry.second.data == b && b->buffer.get() == nullptr) {
        // A fixed_list_size of 0 means we have executed two passes over the
        // file to compute the total length of all (potentially)
        // variable-length lists
        if (fixed_list_size == 0) {
          b->buffer = Buffer(entry.second.cursor->totalSizeBytes);
        } else {
          // otherwise, we can allocate up front and skip the first pass for
          // extra speed.
          auto bytes = entry.second.data->count *
              PropertyTable[entry.second.data->t].stride *
              (entry.second.data->isList ? fixed_list_size : 1);
...
void PlyFile::PlyFileImpl::write(ostream& os, bool _isBinary) {
  if (_isBinary) {
...
void PlyFile::PlyFileImpl::write_binary_internal(ostream& os) {
  isBinary = true;
  write_header(os);

  array<uint8_t, 4> listSize = {0, 0, 0, 0};
  size_t dummyCount = 0;
  ...
  for (auto& e : elements) {
    for (size_t i = 0; i < e.size; ++i) {
      for (auto& p : e.properties) {
        ...
        if (p.isList) {
          memcpy(listSize.data(), &p.listCount, sizeof(uint32_t));
          write_property_binary(p.listType, os, listSize.data(), dummyCount);
...
void PlyFile::PlyFileImpl::write_ascii_internal(ostream& os) {
  write_header(os);
...
void PlyFile::PlyFileImpl::write_header(ostream& os) {
  const locale& fixLoc = locale("C");
  os.imbue(fixLoc);
...
shared_ptr<PlyData> PlyFile::PlyFileImpl::request_properties_from_element(
    const string& elementKey,
    const initializer_list<string>& propertyKeys) {
  // All requested properties in the userDataTable share the same cursor
  // (thrown into the same flat array)
  ParsingHelper helper;
  helper.data = make_shared<PlyData>();
  helper.data->count = 0;
  helper.data->isList = false;
  helper.data->t = Type::INVALID;
  helper.cursor = make_shared<PlyCursor>();
  helper.cursor->byteOffset = 0;
  helper.cursor->totalSizeBytes = 0;

  if (elements.size() == 0) {
    throw runtime_error(
        "parsed header had no elements defined. malformed file?");
  }
  if (!propertyKeys.size()) {
    throw invalid_argument("`propertyKeys` argument is empty");
  }
  if (elementKey.size() == 0) {
    throw invalid_argument("`elementKey` argument is empty");
  }
  ...
  if (elementIndex > -1) {
    ...
    for (auto key : propertyKeys) {
      ...
      if (propertyIndex > -1 && propertyIndex < static_cast<int64_t>(element.properties.size())) {
        ...
        auto result = userData.insert(pair<uint32_t, ParsingHelper>(
            hash_fnv1a(element.name + property.name), helper));
        if (result.second == false) {
          throw invalid_argument(
              "element-property key has already been requested: " +
              to_string(hash_fnv1a(element.name + property.name)));
        }
      } else {
        throw invalid_argument(
            "one of the property keys was not found in the header: " + key);
      }
    }
  } else {
    throw invalid_argument(
...
void PlyFile::PlyFileImpl::add_properties_to_element(
  ...
  auto create_property_on_element = [&](PlyElement& e) {
    for (auto key : propertyKeys) {
      ...
      userData.insert(
          pair<uint32_t, ParsingHelper>(hash_fnv1a(elementKey + key), helper));
      e.properties.push_back(newProp);
    }
  };

  int64_t idx = find_element(elementKey, elements);
  if (idx > -1 && idx < static_cast<int64_t>(elements.size())) {
...
void PlyFile::PlyFileImpl::parse_data(istream& is, bool firstPass) {
  function<size_t(const Type t, void* dest, size_t& destOffset, istream& is)>
      read;
  function<size_t(const PlyProperty& p, istream& is)> skip;

  const auto start = is.tellg();

  if (isBinary) {
    read = [this](const Type t, void* dest, size_t& destOffset, istream& _is) {
      return read_property_binary(t, dest, destOffset, _is);
    };
    skip = [this](const PlyProperty& p, istream& _is) {
      return skip_property_binary(p, _is);
    };
  } else {
    read = [this](const Type t, void* dest, size_t& destOffset, istream& _is) {
      return read_property_ascii(t, dest, destOffset, _is);
    };
    skip = [this](const PlyProperty& p, istream& _is) {
      return skip_property_ascii(p, _is);
    };
  }
...
PlyFile::~PlyFile(){};
bool PlyFile::parse_header(istream& is) {
  return impl->parse_header(is);
}
void PlyFile::read(istream& is, uint32_t fixed_list_size) {
  return impl->read(is, fixed_list_size);
}
void PlyFile::write(ostream& os, bool isBinary) {
  return impl->write(os, isBinary);
}
vector<PlyElement> PlyFile::get_elements() const {
  return impl->elements;
}
vector<string>& PlyFile::get_comments() {
  return impl->comments;
}
vector<string> PlyFile::get_info() const {
  return impl->objInfo;
}
shared_ptr<PlyData> PlyFile::request_properties_from_element(
    const string& elementKey,
    const initializer_list<string>& propertyKeys) {
...
void PlyFile::add_properties_to_element(
    ...
    const Type listType,
    const size_t listCount) {
  return impl->add_properties_to_element(
      elementKey, propertyKeys, type, count, data, listType, listCount);
}
