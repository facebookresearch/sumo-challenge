...
struct PlyFile {
  ...
  /*
   * In the general case where |fixed_list_size| is zero, `read` performs a
   * two-pass parse to support variable length lists. The first pass determines
   * the size of memory to allocate, while the second pass performs the read.
   * The most general use of the ply format is storing triangle meshes. When
   * this fact is known a-priori, we can pass an expected list length that will
   * apply to *all* list elements in a file (e.g. 3). Doing so results in an
   * up-front memory allocation and a single-pass import, generally a ~2x
   * speedup.
   */
